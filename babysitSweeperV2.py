...
if relativeRotation < 45 and relativeRotation > -45: # looking south
    ...
    for i in range(abs(math.ceil((weStart-position[2])/(renderDistanceBlocks)/2))):
        minescript.execute(f"/player {botName}{i} spawn at {position[0]} {botHeight} {position[2]+i*renderDistanceBlocks*2}")
        time.sleep(0.3)
    ...
    for i in range(abs(math.ceil((weStart-position[0])/(renderDistanceBlocks)/2))):
        minescript.execute(f"/player {botName}{i} spawn at {position[0]+i*renderDistanceBlocks*2} {botHeight} {position[2]}")
        time.sleep(0.3)
